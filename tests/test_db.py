...
def test_histogram(prefix, channel, version, dates, metric, value, expected_count):
    ...
    for res in reply["data"]:
        old_pings = expected_count * (NUM_PINGS_PER_DIMENSIONS - NUM_AGGREGATED_CHILD_PINGS) / NUM_PINGS_PER_DIMENSIONS
        new_pings = expected_count * NUM_AGGREGATED_CHILD_PINGS / NUM_PINGS_PER_DIMENSIONS
        assert(res["count"] == new_pings*2 + old_pings*(NUM_CHILDREN_PER_PING + 1))
